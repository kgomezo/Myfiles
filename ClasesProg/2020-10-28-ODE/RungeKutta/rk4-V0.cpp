...
typedef std::vector<double> state_t;  //sinonimo para vector double. se puede para hacer para arrays tambien
using system_t = void(const state_t & y, state_t & dydt, double);  //Puntero a la funcion. Me define mi f, recibe el estado
//las derivadas y el tiempo. funcion de derivadas tiene la misma caracteristica, es un vector estado tambien

void euler(system_t deriv, state_t & data, double t, double h);
void rk4(system_t deriv, state_t & data, double t, double h);
void fderiv(const state_t & y, state_t & dydt, double t); //recibe el estado actual , las der y el t
...
int main(void)
{
    ...
    for (int ii = 0; ii < NSTEPS; ++ii) {
        double t = T0 + ii*H;
        std::cout << t << "  " << y[0] << "   " << y[1] << "\n";
        euler(fderiv, y, t, H); //en cada paso del tiempo llamamos a euler
        //rk4(fderiv, y, t, H);
    }
...
{
    dydt[0] = y[1]; // dx/dt = v
    dydt[1] = -K*std::pow(y[0], 1)/M; // dv/dt = a = -kx/M = -W² x
}
